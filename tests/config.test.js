...
describe('Config', () => {
  ...
  describe('load', () => {
    ...
    it('should throw when required clientId is missing', () => {
      expect(() => Config.load()).toThrow(/Missing required configuration.*TWITCH_CLIENT_ID/);
    });

    it('should throw when clientSecret is required but missing', () => {
      process.env.TWITCH_CLIENT_ID = 'test-client-id';

      expect(() => Config.load({ requireClientSecret: true }))
        .toThrow(/Missing required configuration.*TWITCH_CLIENT_SECRET/);
    });
  });

  describe('validate', () => {
    it('should validate port range', () => {
      expect(() => Config.validate({ port: -1 }, []))
        .toThrow(/PORT must be a valid number between 1 and 65535/);

      expect(() => Config.validate({ port: 70000 }, []))
        .toThrow(/PORT must be a valid number between 1 and 65535/);
    });

    it('should accept valid ports', () => {
      expect(() => Config.validate({ port: 3000 }, [])).not.toThrow();
      expect(() => Config.validate({ port: 1 }, [])).not.toThrow();
      expect(() => Config.validate({ port: 65535 }, [])).not.toThrow();
    });
  });
  ...
  describe('checkComplete', () => {
    it('should return isComplete:true for valid client config', () => {
      process.env.TWITCH_CLIENT_ID = 'test-client-id';
      process.env.TWITCH_ACCESS_TOKEN = 'test-token';
      process.env.TWITCH_BROADCASTER_ID = '12345';

      const result = Config.checkComplete('client');

      expect(result.isComplete).toBe(true);
    });

    it('should return isComplete:false for incomplete config', () => {
      process.env.TWITCH_CLIENT_ID = 'test-client-id';
      // Missing access token and broadcaster ID
