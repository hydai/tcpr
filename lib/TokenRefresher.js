...
import { TokenRefreshError, CredentialErrors } from './errors.js';
  ...
  static async refresh({ refreshToken, clientId, clientSecret }) {
    ...
    try {
      ...
      if (error.response) {
        ...
        // Uses shared CredentialErrors utility for consistent detection
        if (status === 400) {
          // Check for specific "invalid client secret" first (more specific match)
          if (CredentialErrors.isInvalidClientSecret(errorMessage)) {
            throw new TokenRefreshError(
          ...
          // Check for "invalid client" (indicates wrong client ID)
          if (CredentialErrors.isInvalidClientId(errorMessage)) {
            throw new TokenRefreshError(
              'Invalid client ID',
              'invalid_client_id',
              {
