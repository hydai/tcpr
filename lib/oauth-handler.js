 ...
 *
 * @param {number|string|undefined} port - Port value to parse (can be string from env, number, or undefined)
 * @returns {number} Valid port number, defaults to DEFAULTS.PORT (3000) if input is invalid
 * @example
...
export function parsePort(port) {
  ...
  return isNaN(portValue) ? DEFAULTS.PORT : portValue;
}
