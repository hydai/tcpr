...
export class StateTokenManager {
  ...
  /**
   * Create a safe identifier for logging (uses SHA-256 hash)
   * @param {string} token - The token to hash
   * @returns {string} Hashed identifier for safe logging
   * @private
   */
  static _hashForLogging(token) {
  ...
  create(metadata = null) {
    ...
    Logger.debug(`Created state token: ${StateTokenManager._hashForLogging(token)} (expires in ${this.ttl}ms)`);
  ...
  validate(token) {
    ...
    if (!data) {
      Logger.debug(`State token not found: ${StateTokenManager._hashForLogging(token)}`);
      return null;
    }

    // Check expiration
    if (data.expiresAt < Date.now()) {
      Logger.debug(`State token expired: ${StateTokenManager._hashForLogging(token)}`);
      this.tokens.delete(token);
      return null;
    }

    Logger.debug(`State token validated: ${StateTokenManager._hashForLogging(token)}`);
    return data;
  }
  ...
  consume(token) {
    ...
    if (data) {
      this.tokens.delete(token);
      Logger.debug(`State token consumed: ${StateTokenManager._hashForLogging(token)}`);
    }
